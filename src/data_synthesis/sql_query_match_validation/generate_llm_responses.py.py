...
def parse_args():
    ...
    parser.add_argument("--prompt_file", type=str, default='output/prompt/static_requirement_matching.jsonl',
                        help="输入的 prompt 文件路径（支持 .json 或 .jsonl）")
    parser.add_argument("--output_dir", type=str, default="output/llm_raw/static_requirement_matching",
    ...
    parser.add_argument("--id_fields", nargs="*", default=['table_name', 'create_sql'],
                        help="用于判断是否已处理的唯一标识字段（多个字段组合）")
    parser.add_argument("--prompt_field", type=str, default='static_requirement_matching.prompt',
