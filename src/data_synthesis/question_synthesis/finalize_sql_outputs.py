...
def parse_args():
    ...
    parser.add_argument("--output_file", type=str, default="output/final/annotated_ddl.jsonl",
                        help="最终输出文件路径")
    parser.add_argument("--fields", nargs="+", default=["table_name", "create_sql",
                                                        "sample_data", 'annotated_ddl',
                                                        'table_summary', 'similar_tables',
                                                        'function_descriptions', 'prompt_context',
                                                        'synthesis_sql', 'question_synthesis_metadata',
                                                        'structured_response'
                                                        ],
                        help="需要保留的字段列表（从输入记录中提取）")
    parser.add_argument("--rename_from", type=str, default='structured_response',
                        help="要重命名的源字段名（若该字段在 --fields 中且存在，则会被重命名）")
    parser.add_argument("--rename_to", type=str, default="synthesis_question",
